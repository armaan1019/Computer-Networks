...
Each node runs this program, and they exchange cost information to compute the shortest paths to all other nodes. 
The program uses threads to handle receiving updates and running the link state algorithm concurrently. 
The user can also input changes to the cost of links, which will be propagated to other nodes.
*/
...
void * receive_info (void *arg)
{
	...
	while(1) {
		...
		pthread_mutex_lock(&lock);
		costs[rid][nid] = newcost;
		costs[nid][rid] = newcost;
		pthread_mutex_unlock(&lock);

		printf("Updated cost %d-%d = %d\n", rid, nid, newcost);
		print_costs();
	}
