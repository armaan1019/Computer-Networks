...
* Title: Lab5 - UDP server which has reliable data transfer with checksum, sequence number, acknowledgement number, and timer
* Description: This program implements a UDP server that receives packets with a checksum, sequence number, and acknowledgement number from a client. 
The server sends acknowledgements back to the client and uses a timer to handle timeouts. 
The program simulates packet loss and handles retransmissions accordingly.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void serverSend(int sockfd, const struct sockaddr *address, socklen_t addrlen, int seqnum) {
    ...
    if (rand() % 5 == 0){ // this give a probability of 20% for a loss of ACK 
        ...
        //send packet
        sendto(sockfd, &packet, sizeof(Header), 0, address, addrlen);
