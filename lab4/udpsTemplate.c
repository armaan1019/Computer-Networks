...
* Title: Lab4 - Part 1 - UDP Server
* Description: This program implements a UDP server that receives data from a UDP client and writes it to a file.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void serverSend(int sockfd, const struct sockaddr *address, socklen_t addrlen) {
    ...
    //send packet
    sendto(sockfd, &packet, sizeof(packet), 0, address, addrlen); // send packet using udp socket, using address of client
...
int main(int argc, char *argv[]) {
    ...
    memset(&servAddr, 0, sizeof(servAddr)); //clear memory segment
    servAddr.sin_family = AF_INET; //IPv4
    servAddr.sin_port = htons(atoi(argv[1])); //port number
    servAddr.sin_addr.s_addr = INADDR_ANY; //any incoming address
